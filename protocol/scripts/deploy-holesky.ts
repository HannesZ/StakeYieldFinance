/**
 * Deploy StableYield protocol to Holesky testnet.
 *
 * Usage:
 *   npx hardhat run scripts/deploy-holesky.ts --network holesky
 *
 * Env vars:
 *   DEPLOYER_KEY         — private key for deployment
 *   ETHERSCAN_API_KEY    — optional, for contract verification
 *   USE_MOCK_WSTETH      — set "true" to deploy MockWstETH instead of using real Holesky wstETH
 */
...
const HOLESKY_WSTETH = "0x8d09a4502Cc8Cf1547aD300E066060D043f6982D";
...
async function main() {
  ...
  console.log("═══════════════════════════════════════════════════════════");
  console.log("  StableYield Protocol — Holesky Deployment");
  console.log("═══════════════════════════════════════════════════════════");
  ...
  if (process.env.USE_MOCK_WSTETH === "true") {
    ...
    wstETHAddress = HOLESKY_WSTETH;
    console.log(`  [1/7] Using Holesky wstETH: ${wstETHAddress}`);
  }
  ...
  Next steps:
    ...
    3. Verify contracts on Etherscan:
       npx hardhat verify --network holesky ${vaultAddr} ${deployerAddr} ${wstETHAddress} ${syLSTAddr} ${reserveAddr} ${spreadAddr}
  `);
}
